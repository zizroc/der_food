...
food_pathways_tmp <- harmonized_food_data_targets %>% 
  ...
  dplyr::mutate(tonnes_yr_interp = spline(
    x = year,
    y = tonnes_yr,
    xout = year,
    ties = "ordered"
...
Take first and second differences from pathways.
```{r}
...
Take first and second differences. Second difference is equivalent to curvature.
```{r}
# food_curv <- food_hist_countries %>%
#   # filter(area_code == 2, model_group == "cereals") %>%
#   dplyr::group_by(area_code, area, model_group) %>%
#   dplyr::arrange(year) %>%
...
NB I don't think it's setting the target year correctly
```{r}
...
# ```{r}
# Dens1 <- density(dens$diff2)
# Dens_df <- data.frame(curv = Dens1$x, dens = Dens1$y)
# ```
