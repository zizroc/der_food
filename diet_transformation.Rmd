...
Load healthy food demand data.

```{r}
# readr::write_csv(ssp2_kcal_edu, file = paste0(data_path, "/ssp2_kcal_edu.csv"))
ssp2_kcal_edu <- readr::read_csv(file = paste0(data_path, "/ssp2_kcal_edu.csv"))
```
...
Load future pathway kcal data, including diet plans calculated in `iep_healthy_food.Rmd`.

```{r}
# readr::write_csv(all_plans_long, file = paste0(data_path, "/diet_plans_long.csv"))
all_plans_long <- readr::read_csv(file = paste0(data_path, "/diet_plans_long.csv"))
```
...
fao_kcal_diff_ranges %>% 
  dplyr::filter(year %in% c(2050, 2075)) %>% 
  dplyr::group_by(year, food_source, diet_plan_f) %>% 
  dplyr::summarise(diff_max = max(diff_max) * 1e-6, 
                   diff_min = min(diff_min) * 1e-6, 
                   pc_diff_max = max(pc_diff_max), 
                   pc_diff_min = min(pc_diff_min), 
                   .groups = "drop") %>% 
  dplyr::filter(diet_plan_f == "USDA" & food_source == "animal_products")
...
```{r}
food_crop_proj <- readr::read_csv(file = paste0(data_path, "/food_crop_proj.csv"))
food_animal_proj <- readr::read_csv(file = paste0(data_path, "/food_animal_proj.csv"))
feed_total_proj <- readr::read_csv(file = paste0(data_path, "/feed_crop_animal_proj.csv"))
```

```{r}
scenarios_df <- data.frame(
   SCENARIO = c("SSP1_v9_130115", "SSP2_v9_130115", "SSP3_v9_130115", "SSP4d_v9_130115", "SSP5_v9_130115"), 
...
food_proj_tmp2 <- food_animal_proj %>%
    dplyr::filter(iso_alpha3 %in% iso_list_harmonized) %>%
    dplyr::mutate(tonnes_yr = tonnes_day_food * 365) %>% 
  dplyr::full_join(
    scenarios_df
...
food_proj <- food_proj_tmp1 %>% 
  dplyr::full_join(food_proj_tmp2)
```

Do the same for feed.
...
```{r}
food_feed_proj_diff %>% 
  dplyr::filter(scenario == "SSP3" & source_group == "plant_products" & diet_plan == "USDA" & crop_class == "feed")
```
...
```{r}
readr::write_csv(food_pathways_clean, file = paste0(data_path, "/food_pathways_clean.csv"))
readr::write_csv(feed_pathways_clean, file = paste0(data_path, "/feed_pathways_clean.csv"))
```
...
```{r}
readr::write_csv(Food_feed_pathways_curves, 
                 file = paste0(data_path, "/pathways_roc.csv"))
readr::write_csv(Food_feed_curv_regions, 
                 file = paste0(data_path, "/pathways_roc_regions.csv"))
```
