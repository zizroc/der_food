...
Now plot in terms of relative change from demand in 2018.

```{r}
totals_2018 <- all_plans_long %>% 
  dplyr::mutate(M_kcal_day_2018 = ifelse(year == 2018, M_kcal_per_day, NA)) %>% 
...
diff <- global_crop_demand_food_feed %>% 
  dplyr::mutate(tonnes_yr_2018 = ifelse(year == 2018, 
                tonnes_yr, 
                NA)) %>% 
