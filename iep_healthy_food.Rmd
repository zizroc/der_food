...
```{r}
ssp_base <- read.csv(file = paste0(data_path, "/SspDb_country_data_2013-06-12.csv"), 
                     header = TRUE, 
...
If necessary, load SSP population data frame wrangled earlier.
```{r}
# output_path <- "/home/thomson/Projects/iep_food/IEP_food_demand_files/"
...
```{r}
grDevices::jpeg(filename = "Plots/figure_S1a.jpg",
               width = 750,
               height = 500,
               units = "px",
               quality = 1)
...
Bind these dfs by row. Now `kcal_edu` is a df of average kcal demand, with a scaling factor of millions of persons per year, by year, SSP, country, sex, education level, age bracket, and activity level.
```{r}
...
if(!exists("loc_map")) {
  loc_map <- readr::read_csv(
    paste0(
      data_path, "/location_code_map_data.csv"
    ), 
    col_types = cols(
...
Now plot these. Factored regions are added for plotting.
```{r}
...
grDevices::jpeg(filename = "Plots/figure_S1b.jpg", 
               width = 750, 
               height = 500, 
               units = "px", 
               quality = 1)
...
If FAO DER remains the same, the future deficit will look like:
```{r}
...
der_ssp0 <- ssp2_kcal_edu %>%
  ...
  dplyr::summarise(der_ave = sum(wtd_mean),
                   .groups = "drop") %>%
  dplyr::filter(year %in% c(2020, 2050, 2075))

der_def <- der_fao_2020 %>% 
  dplyr::full_join(der_ssp0) %>% 
  dplyr::mutate(der_deficit = DER_FAO - der_ave) %>% 
  dplyr::full_join(regions_map) %>% 
  dplyr::select(-Unit, -DER_FAO, -subregionname, -der_ave, -countryname) %>% 
  tidyr::drop_na(region)

```

Order the regions by DER deficit.
```{r}
der_def <- der_def %>% 
  dplyr::mutate(region_f = region)

der_def$region_f <- factor(der_def$region_f, levels = c("South Asia", "Africa", "Oceania", "Southeast Asia", "South America", "Central Asia and Russia", "Latin America and Caribbean", "MENA", "East Asia", "Europe", "Australia and New Zealand", "North America"))

```

```{r}
region_labs <- data.frame(
  region_f = c("South Asia", "Africa", "Oceania", "Southeast Asia", "South America", "Central Asia and Russia", "Latin America and Caribbean", "MENA", "East Asia", "Europe", "Australia and New Zealand", "North America"), 
  region_labs = c("South Asia", "Sub-Sahran Africa", "Oceania", "Southeast Asia", "South America", "Central Asia and\nRussia", "Latin America and\nthe Caribbean", "Middle East and\nNorth Africa", "East Asia", "Europe", "Australia and\nNew Zealand", "North America")
)

der_def <- der_def %>% 
  full_join(region_labs)

der_def$region_labs <- factor(der_def$region_labs, levels = c("South Asia", "Sub-Sahran Africa", "Oceania", "Southeast Asia", "South America", "Central Asia and\nRussia", "Latin America and\nthe Caribbean", "Middle East and\nNorth Africa", "East Asia", "Europe", "Australia and\nNew Zealand", "North America"))
...
```{r}
p_fig1 <- ggplot(data = der_def,
                 mapping = aes(x = region_labs,
                               y = der_deficit)) +
  geom_hline(yintercept = 0, alpha = 0.9) +
  geom_boxplot(mapping = aes(fill = as.factor(year)),
               position = "dodge") +
  scale_fill_brewer(labels = region_labs, palette = 1) +
  theme_minimal() +
  theme(
    legend.position = "top",
...
```{r}
p_fig1
grDevices::jpeg(filename = "Plots/figure_1.jpg", 
               width = 750, 
               height = 500, 
               units = "px", 
               quality = 1)
...
```{r}
diet_plan_labels <- data.frame(
  diet_plan = c("ELA-PHD", "CNS-CFP", "FDA-SDP"), 
  diet_plan_lab = c("ELA", "CNS", "USDA")
)

model_group_labels <- data.frame(
  model_group = c("beef_lamb_pork", "cereals", "chicken_poultry", "dairy", "eggs", "fish_seafood", "fruits", "legumes", "oils", "roots_tubers", "sugarcrops", "treenuts", "vegetables", "meat_poultry"), 
  model_group_lab = c("meat & poultry", "cereals", "meat & poultry", "dairy", "eggs", "fish & seafood", "fruits", "legumes", "vegetable oils", "roots & tubers", "sugarcrops", "treenuts", "vegetables", "meat & poultry")
)

model_group_label_colours <- data.frame(
  model_group_lab = c("meat & poultry", "cereals", "dairy", "eggs", "fish & seafood", "fruits", "legumes", "vegetable oils", "roots & tubers", "sugarcrops", "treenuts", "vegetables"), 
    model_group_lab_col = c("lightcoral", "lightgoldenrod", "lightblue", "cornsilk2", "deepskyblue4", "mediumseagreen", "khaki3", "darkolivegreen4", "lightgoldenrod4", "lightgreen", "darkorange4", "darkgreen")
)

all_plans_for_plot <- all_plans_mapped %>% 
  dplyr::full_join(diet_plan_labels) %>% 
  dplyr::full_join(model_group_labels) %>% 
  tidyr::drop_na() %>% 
  dplyr::group_by(diet_plan_lab, model_group_lab) %>% 
  dplyr::summarise(slice = sum(kcal_ratio, na.rm = TRUE), 
                   .groups = "drop")
  
```

```{r}
all_plans_for_plot$diet_plan_lab_f <- all_plans_for_plot$diet_plan_lab
all_plans_for_plot$diet_plan_lab_f <- factor(all_plans_for_plot$diet_plan_lab, levels = c("ELA", "CNS", "USDA"))
```


```{r}
ggplot(all_plans_for_plot, 
       aes(x = "", y = slice, fill = model_group_lab)) + 
  geom_bar(stat = "identity", width = 1, size = 0.2, colour = "white") + 
  coord_polar("y", start = 0) + 
  scale_fill_manual(values = model_group_label_colours$model_group_lab_col, labels = model_group_label_colours$model_group_lab) + 
...
kcal_edu_fda <- kcal_edu %>% 
  ...
  dplyr::mutate(animal_products = meat_poultry + dairy, 
                plant_products = cereals + fruits + oils + roots_tubers + sugarcrops + treenuts + vegetables + legumes) %>% 
  dplyr::select(SCENARIO, REGION, diet_plan, year, animal_products , fish_seafood, plant_products)
...
Now plot in terms of relative change from demand in "2018" (i.e., 2020 in the data).

```{r}
totals_2018 <- all_plans_long %>% 
  dplyr::mutate(M_kcal_day_2018 = ifelse(year == 2020, M_kcal_per_day, NA)) %>% 
  dplyr::group_by(SCENARIO, diet_plan) %>% 
  dplyr::filter(year == 2020) %>% 
  dplyr::summarise(total_kcal_2018 = sum(M_kcal_per_day, na.rm = TRUE), .groups = "drop")

diff_kcal_to_2018 <- all_plans_long %>%
  dplyr::mutate(M_kcal_day_2018 = ifelse(year == 2020, M_kcal_per_day, NA)) %>%
  dplyr::group_by(SCENARIO, diet_plan, food_source) %>%
  tidyr::fill(M_kcal_day_2018, .direction = "updown") %>%
  dplyr::ungroup() %>%
  dplyr::mutate(diff_kcal_day = M_kcal_per_day - M_kcal_day_2018) %>%
  dplyr::select(-M_kcal_per_day,-M_kcal_day_2018) %>%
  dplyr::left_join(totals_2018, by = c("SCENARIO", "diet_plan")) %>% 
  dplyr::mutate(rel_diff_kcal_day = diff_kcal_day/total_kcal_2018)

```


Plot shows the relative change in daily kcal demand compared to 2018 for the different SSPs.
```{r}
library(scales)

# ymax <- (max(diff_kcal_to_2018$rel_diff_kcal_day)+0.001*max(diff_kcal_to_2018$rel_diff_kcal_day)) 
# ymin <- (min(diff_kcal_to_2018$rel_diff_kcal_day)-0.001*min(diff_kcal_to_2018$rel_diff_kcal_day)) 
ymax <- 0.5
ymin <- -0.05
...
value_chain_loss <-
  readr::read_csv(
    file = paste0(data_path, "/Alexander_etal_2017_food_waste - summary.csv"),
...
Source: [FAOSTAT Food Balances.](https://fenixservices.fao.org/faostat/static/bulkdownloads/FoodBalanceSheets_E_All_Data_(Normalized).zip).

```{r}
if(!exists("food_balance_data")){
  food_balance_data <- read.csv(file = paste0(data_path, "/FoodBalanceSheetsHistoric_E_All_Data_(Normalized).csv"), 
                                header = TRUE, 
                                stringsAsFactors = FALSE) %>% 
...
if(!exists("combined_item_group_mapper")){
  source(file = paste0(data_path, "/FAO_item_group_mapper.R")) #combined_item_group_mapper
}
...
Summarise by food, feed, seed, etc. categories.
```{r}
...
test_2018 <- test %>%
  dplyr::filter(year == 2020) %>%
  dplyr::mutate(tonnes_year_2018 = tonnes_year)

test <- test %>%
  dplyr::filter(year >= 2020) %>%
  dplyr::left_join(test_2018,
                   by = c("SCENARIO", "diet_plan", "year", "tonnes_day", "tonnes_year")) %>%
  dplyr::group_by(SCENARIO, diet_plan) %>%
  tidyr::fill(tonnes_year_2018) %>% 
  dplyr::ungroup() %>% 
  dplyr::group_by(SCENARIO, diet_plan, year) %>% 
  dplyr::summarise(surplus = (tonnes_year - tonnes_year_2018), 
                   .groups = "drop")

head(test)
```

What is the global demand for animal products as food and feed?
```{r}
global_livestock_demand_as_feed <- feed_ingredient_demand_summary %>% 
...
diff <- global_crop_demand_food_feed %>% 
  dplyr::mutate(tonnes_yr_2018 = ifelse(year == 2020, 
                tonnes_yr, 
                NA)) %>% 
...
```{r}
readr::write_csv(food_weight, file = paste0(data_path, "/food_crop_proj.csv"))
readr::write_csv(live_food_weight, file = paste0(data_path, "/food_animal_proj.csv"))
readr::write_csv(feed_ingredient_demand_summary, file = paste0(data_path, "/feed_crop_animal_proj.csv"))
```
